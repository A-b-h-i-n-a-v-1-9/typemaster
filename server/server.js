...
// Create HTTPS agent to ignore SSL errors (dev only)
...
// Fetch prompt (tries API, falls back to local)
async function fetchPrompt(mode = "quote") {
    try {
        ...
        const i = Math.floor(Math.random() * fallbackPrompts.length);
        return fallbackPrompts[i];
    }
}
